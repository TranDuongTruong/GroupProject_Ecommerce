...
namespace GroupProject_Ecommerce
{
            ...
            var parametersImages = new Dictionary<string, object>
    {
        { "@ProductID", productId }
            ...
            string sqlSimilarProducts = "SELECT TOP 8 ProductName, Price, ImageName FROM Products WHERE ShopID = @ShopID AND ProductID <> @ProductID ORDER BY NEWID()";
            var parametersSimilar = new Dictionary<string, object>
    {
        ...
        { "@ProductID", productId }
                ...
                dtProductDetail.Rows.Count > 0 && dtMainImage.Rows.Count > 0 && dtThumbnails.Rows.Count > 0)
            {
                ...
                // Gán dữ liệu cho các controls trên giao diện
